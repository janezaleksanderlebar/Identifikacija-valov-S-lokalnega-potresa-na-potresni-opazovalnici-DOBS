...
#------------------------------------------------------------------------------
# Uporaba SOS filtra PSD, za Z komponento
...
# 1 VRH
    ...
    num, denum = signal.iirfilter(2,[5,12.8] , rs=60, btype='band', fs=200 ,analog=False, ftype='butter')
    sos_band=signal.tf2sos(num, denum, analog=False)
...
if input_2 == "SOS_FILTER_PSD":
    ...
    plt.semilogx(w,  20 * np.log10(abs(h)),linewidth=1.5,label='{x}'.format(x="Inverzni resonančni filter"))
